...
import { update_filters, clear_filters } from '../redux/slices/filterSlice'
...
const Filters = () => {
	...
	const handleSubmit = (e) => {
		...
		dispatch(update_filters({ [name]: value }))
	}
				...
				<form onSubmit={handleSubmit}>
						...
						<div>
							...
							{categories.map((c, index) => {
								return (
									<button
										key={index}
										onClick={() => dispatch(update_filters({ category: c }))}
										type='button'
										className={`${
					...
					<div className='form-control'>
						...
						<select
							...
							value={company}
							onChange={(e) =>
								dispatch(update_filters({ company: e.target.value }))
							}
							className='company'
						>
							{/* c means company */}
							...
							{colors.map((c, index) => {
								if (c === 'all') {
									return (
										<button
											key={index}
											onClick={() => dispatch(update_filters({ color: c }))}
											className={`${
					...
					<div className='form-control'>
						...
						<input
							...
							onChange={(e) =>
								dispatch(update_filters({ price: +e.target.value }))
							}
							min={min_price}
							max={max_price}
						/>
						...
						<div className='form-control shipping'>
							...
							<input
								...
								id='shipping'
								onChange={() =>
									dispatch(update_filters({ shipping: !shipping }))
								}
								checked={shipping}
							/>
						</div>
